...
import type { Skill, Run, Secret, SkillOutput, WalletSnapshot } from '../lib/types'
...
export default function Dashboard() {
  ...
  const [view, setView] = useState<'hq' | 'secrets' | 'economics'>('hq')
  ...
  const flash = (msg: string, tone: 'ok' | 'err' = 'ok') => { setToast(msg); setToastTone(tone); setTimeout(() => setToast(''), tone === 'err' ? 5000 : 3000) }
  ...
  const syncToGithub = async () => { setSyncing(true); try { const r = await fetch('/api/sync', { method: 'POST' }); const d = await r.json().catch(() => ({} as { error?: string })); if (r.ok) { flash('Deployed'); setHasChanges(false) } else { flash(d?.error || 'Deploy failed') } } catch { flash('Deploy failed') } finally { setSyncing(false) } }
  const pullFromGithub = async () => { setPulling(true); try { const r = await fetch('/api/outputs', { method: 'POST' }); const d = await r.json().catch(() => ({} as { error?: string; warning?: string })); if (r.ok) { flash('Pulled'); setFeedKey(k => k + 1); fetchData() } else if (r.status === 207) { flash(d?.warning || 'Pulled with conflicts') } else { flash(d?.error || 'Pull failed') } } catch { flash('Pull failed') } finally { setPulling(false) } }
  ...
  return (
    <div className="min-h-screen flex flex-col mission-root text-primary-100">
      {toast && <div className={`fixed bottom-4 left-1/2 -translate-x-1/2 z-50 px-5 py-2.5 text-xs font-mono tracking-wide shadow-xl ${toastTone === 'err' ? 'bg-eva-red/90 text-white' : 'bg-eva-black text-white'}`}>{toast}</div>}
      ...
      <LeftSidebar
        view={view}
        onNavigate={(next: 'hq' | 'wallet' | 'secrets' | 'economics') => {
          if (next === 'wallet') {
            router.push('/wallet')
          } else {
            setView(next)
      ...
      {showImport && <ImportModal onClose={() => setShowImport(false)} onImport={importSkill} onRecruited={(info) => { flash(`${displayName(info.name)} recruited`); setHasChanges(true); fetchData() }} />}
