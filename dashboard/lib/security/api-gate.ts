 ...
 * `http://localhost:5555` via `./claw`. The `/api/*` routes hold the
 * keys to the GitHub Actions kingdom for this repo:
 *
 *   - `POST /api/skills/[name]/run`        — triggers `gh workflow run claw.yml`
 ...
 *      Defeats #2.
 *
 * The hatches `CLAWTRL_DASHBOARD_ALLOWED_HOSTS` (comma-separated extra
 * hosts) and `CLAWTRL_DASHBOARD_ALLOW_ANY_HOST=1` (full bypass, intended
...
export function gateRequest(req: {
  ...
  const opts: GateOptions = {
    extraAllowed: parseAllowedHosts(
      process.env.CLAWTRL_DASHBOARD_ALLOWED_HOSTS ||
      process.env.AEON_DASHBOARD_ALLOWED_HOSTS,
    ),
    allowAny:
      process.env.CLAWTRL_DASHBOARD_ALLOW_ANY_HOST === "1" ||
  ...
  if (!isAllowedHost(req.headers.get("host"), opts)) {
    return new Response(
      JSON.stringify({
        ...
        hint:
          "The Clawtrl dashboard API accepts loopback Hosts only (127.0.0.1, localhost, ::1) by default. " +
          "If you're fronting the dashboard at a non-loopback hostname (LAN, Tailscale, .local), add it to " +
          "CLAWTRL_DASHBOARD_ALLOWED_HOSTS (comma-separated). For trusted reverse-proxy setups that terminate " +
          "Host upstream, set CLAWTRL_DASHBOARD_ALLOW_ANY_HOST=1 — this disables the gate, do not use it on a " +
