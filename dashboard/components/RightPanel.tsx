...
export function RightPanel({ runs, outputs, feedLoading, analyticsData, onViewRun, onRefresh, onFetchAnalytics }: RightPanelProps) {
  ...
  return (
    <div className="w-[320px] border-l border-[rgba(228,236,255,0.12)] flex flex-col shrink-0 bg-[rgba(5,12,32,0.65)] backdrop-blur-xl">
      <div className="h-12 border-b border-[rgba(228,236,255,0.12)] flex items-center px-2 gap-0.5 shrink-0">
        {([['feed', 'Transmissions'], ['runs', 'Missions'], ['analytics', 'Telemetry']] as const).map(([tab, label]) => (
          <button key={tab} onClick={() => { setRightTab(tab); if (tab === 'analytics') onFetchAnalytics() }}
            className={`text-[10px] px-1.5 py-1.5 transition-colors font-mono uppercase tracking-[0.5px] border-b-2 ${rightTab === tab ? 'border-[rgba(51,240,183,0.85)] text-white' : 'border-transparent text-primary-40 hover:text-primary-100'}`}>{label}</button>
        ))}
        <button onClick={onRefresh} title="Refresh" aria-label="Refresh" className="ml-auto w-7 h-7 flex items-center justify-center rounded text-primary-40 hover:text-[rgba(51,240,183,0.95)] hover:bg-[rgba(51,240,183,0.08)] transition-colors">
            ...
            {!runs.length ? <div className="px-4 py-12 text-center text-xs text-primary-35 font-mono">No activity yet</div> :
              runs.map(run => (
                <button key={run.id} onClick={() => handleViewRun(run)}
                  className="w-full flex items-center gap-2.5 px-3 py-2.5 border-b border-[rgba(228,236,255,0.08)] hover:bg-[rgba(255,255,255,0.06)] transition-colors text-left">
        ...
        {rightTab === 'runs' && (
          selectedRun ? (
            <div className="flex flex-col h-full">
              <div className="flex items-center gap-2 px-3 py-2.5 border-b border-[rgba(228,236,255,0.12)]">
                ...
                <a href={selectedRun.url} target="_blank" rel="noopener noreferrer" className="text-[11px] text-primary-40 font-mono border border-[rgba(228,236,255,0.18)] px-2 py-0.5 hover:border-eva-orange hover:text-eva-orange transition-colors">GitHub</a>
              </div>
