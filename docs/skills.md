...
Aeon ships with 50+ skills. Each skill is a self-contained markdown instruction file in `skills/<name>/SKILL.md`. Enable any skill in `claw.yml` and it runs on schedule.
...
Installed skills land in `skills/` and appear in `claw.yml` as disabled. Flip `enabled: true` to activate.
