...
Read this running instance's local `claw.yml` once. Build:
...
```bash
gh api "repos/${FORK_FULL}/contents/claw.yml?ref=${FORK_DEFAULT_BRANCH}" --jq '.content' | base64 -d
```

Error handling:
- 404 / 409 (empty repo): mark `status: "no_tree"`, skip claw.yml extraction, continue.
- 403 with `X-RateLimit-Remaining: 0`: sleep 60s, retry once. If still failing, mark `status: "rate_limited"` and continue.
- Tree contains claw.yml but contents call 404s: mark `status: "yml_unreadable"`, continue.
- claw.yml present but YAML parse fails: mark `status: "yml_invalid"`, continue.

For each readable `claw.yml`, extract per-skill `{enabled, model, var, schedule}`. Treat missing keys as inheriting the upstream default (do NOT count those as overrides).
...
- **TEMPLATE**: claw.yml readable but every signal is 0 — exclude from divergence math
- **UNREADABLE**: no_tree / no claw.yml / yml_unreadable / yml_invalid / rate_limited — tracked in source-status footer
...
*Scanned ${N_ACTIVE} active forks of ${TARGET_REPO} (pushed in last 30 days). ${N_CONFIGURED} are configured (claw.yml diverges from upstream defaults). Divergence scored against the configured ${N_CONFIGURED}.*
...
---
*Source: GitHub API — forks of ${TARGET_REPO}. Methodology: a fork is "configured" if its claw.yml diverges from upstream defaults on enabled, model, var, or schedule for any skill. Untouched templates are excluded from divergence math. Companion to skill-leaderboard (popularity) and fork-fleet (per-fork work).*
```
