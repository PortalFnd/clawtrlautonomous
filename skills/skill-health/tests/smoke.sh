...
validate_skill() {
  ...
  if [[ -n "$secret_refs" ]]; then
    while IFS= read -r ref; do
      ...
      if [[ "$known" == "false" ]]; then
        warn "references unknown variable '\$$ref' — verify it's in claw.yml env or a known secret"
      fi
    done <<< "$secret_refs"
  fi

  # 7. Validate cron expression if skill is registered in claw.yml with a schedule
  if [[ -f "$AEON_YML" ]]; then
    local cron_expr
    ...
    if [[ -n "$cron_expr" ]]; then
      ...
      if [[ "$field_count" -ne 5 ]]; then
        fail "invalid cron expression in claw.yml for $skill_name: '$cron_expr' (expected 5 fields, got $field_count)"
        skill_ok=false
      else
