...
- Read `claw.yml` and build a set `ENABLED` of skill names where the entry has `enabled: true`. This drives the priority calculation in step 5.
...
### {skill_name} — {priority}
- **Source:** {source_repo} at {source_path} (branch: {branch}; claw.yml: {ENABLED|DISABLED})
...
7. Notify:
   ...
   *Skill update accepted* {skill_name} advanced from {old_sha[:7]} to {new_sha[:7]} (security: {verdict}).
   Re-enable in claw.yml if needed.
   ```
...
## skill-update-check
- Mode: AUDIT | ACCEPT
- Tracked: N (enabled in claw.yml: M)
