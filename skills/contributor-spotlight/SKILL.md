...
```bash
# Read the fork's claw.yml to count enabled skills, and diff against parent's
gh api "repos/${FEATURED_FORK}/contents/claw.yml?ref=${FORK_DEFAULT_BRANCH}" \
...
```bash
ENABLED_SKILLS=$(grep -E '^[[:space:]]+[a-zA-Z0-9_-]+:[[:space:]]*\{[^}]*enabled:[[:space:]]*true' /tmp/fork-claw.yml \
