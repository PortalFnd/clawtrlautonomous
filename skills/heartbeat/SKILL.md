...
Read `claw.yml` for enabled skills with schedules. Cross-reference with `memory/cron-state.json`:
...
- `claw.yml` — enabled skill list with schedules
...
### Rules
- Include **all** enabled skills from `claw.yml` (not only those with recent runs). For skills with no entry in cron-state.json, show `—` for timestamp and `not yet run` in status.
...
- Never expose values from `.env`, secrets, or anything outside cron-state.json + issues/INDEX.md + claw.yml + articles/token-report-*.md. This file is public.
