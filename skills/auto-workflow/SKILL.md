...
description: Analyze a URL and generate a tailored claw.yml schedule with skill suggestions
var: ""
tags: [meta, dev]
---
...
3. Emits a *delta* against current `claw.yml` rather than a full config dump.
...
- `claw.yml` — CURRENT skill enablement, `var`, `schedule`, `model` per skill (this is the comparison baseline)
...
### 4. Compare against current claw.yml (delta, not dump)

For each tiered recommendation, compute the delta:

| Recommended state | Current state in claw.yml | Action |
|-------------------|--------------------------|--------|
