...
```json
{
  ...
  "checks": [
    { "status": "pass", "check": "workflow .github/workflows/claw.yml", "detail": "present", "fix": "" },
...
- **`${next_action}`** — derived from the highest-priority gap:
  ...
  - all clean → `"Nothing — you're done. Trigger your first skill: gh workflow run claw.yml -f skill=heartbeat"`
