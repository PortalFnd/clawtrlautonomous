...
${fingerprint_2} severity=High status=manual rule=unpinned-uses file=.github/workflows/claw.yml step=Checkout
...
-->
```
...
- No new env vars beyond `GITHUB_TOKEN` / `GH_GLOBAL` (already in `claw.yml`).
