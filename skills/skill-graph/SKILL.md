...
**Explicit edges:**
- `claw.yml` → per-skill `enabled`, `schedule`, `var`, `model`; `chains:` blocks (`steps:`, `consume:`, `parallel:`); `reactive:` blocks (`trigger:`, `on:`, `when:`)
...
- Enabled state comes from `claw.yml` only — never infer from cron-state or recent runs (a skill can be enabled but not yet run).
