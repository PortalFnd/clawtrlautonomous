...
claw.yml             — `enabled: true|false` flags per skill (skills surfaced as MCP tools follow `mcp-server/src/index.ts` which exposes ALL skills, not just enabled ones — match that behavior)
...
Auto-generated by skills/smithery-manifest from skills.json + claw.yml.
...
Pure local file I/O. Reads `skills.json`, `claw.yml`, `mcp-server/package.json`, and `README.md`; writes three files under `docs/`; runs `gh pr create` (uses `gh`'s built-in auth). No `curl`, no env-var-in-headers expansion, no prefetch or postprocess scripts needed.
...
- **Idempotent.** Running multiple times with no changes to `skills.json` / `claw.yml` / `mcp-server/package.json` / `README.md` produces identical output and triggers no PR and no notification.
