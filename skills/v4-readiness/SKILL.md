---
name: v4-readiness
description: Generate a per-fork v4 upgrade readiness checklist — reads the fork's claw.yml, skills.json, and MEMORY.md, cross-references against the embedded v4 change manifest, emits Safe / Review / Custom / Action-items breakdown
var: ""
tags: [meta, dx]
---

> **${var}** — Optional. Pass `dry-run` to skip the notification (article still writes, log still appends). Pass a fork repo slug (e.g. `someuser/aeon`) to read remote `claw.yml` + `skills.json` from that fork instead of the local working tree (useful for surveying the fleet ahead of a v4 announcement). Empty = audit the local fork.
...
v4 is announced as a full redesign (~2 weeks lead time per operator's social posts). 40+ forks are running on the current architecture. Without a structured per-fork readiness check, operators hit breaking changes blind: they pull the upstream, their custom `claw.yml` contains a now-removed key, a chain consumer references a renamed skill, a model override points to a retired model, their custom skill imports from a path that moved. Every one of those is recoverable in five minutes if it's surfaced ahead of time and unrecoverable in five hours if it's discovered at the moment a cron fires.
...
Reads:
- `claw.yml` — enabled skills, model overrides, chain definitions, schedule strings, reactive triggers, gateway block, channels block.
...
|---------|----------------|--------------------|
| `chains:` runner interface | `claw.yml` | Step format (`parallel:` / `consume:` keys, `on_error` semantics) |
| `reactive:` trigger conditions | `claw.yml` | Condition vocabulary (`consecutive_failures`, `success_rate`, `last_status`) |
| Chain runner output passing | `.outputs/*.md`, `chain-runner.yml` | Path layout, file shape |
| Schedule syntax (`workflow_dispatch`, `reactive`, cron) | `claw.yml` | Naming of pseudo-schedules; cron escapes |
| Model selector strings | `claw.yml` per-skill `model:` | Model id references — Opus/Sonnet/Haiku version pins |
| `gateway:` provider block | `claw.yml` | Bankr/direct selector, env var names |
| `channels:` block (`jsonrender.enabled`) | `claw.yml` | Toggle key names, channel set |
...
|-------|-------|------------------------|
| `claw.yml` | direct read | `gh api repos/${TARGET}/contents/claw.yml --jq .content \| base64 -d` |
...
If `claw.yml` is unreadable, log `V4_READINESS_NO_CONFIG` and exit with no notification (the fork is not initialized; nothing to check).

### 3. Compute the enabled-skill snapshot

From `claw.yml`, extract for each skill under `skills:`:
- `enabled` (true/false)
...
| Tag | Heuristic |
|-----|-----------|
| `trivial` | Config rename only; one-line `claw.yml` edit |
...
*Audit basis: claw.yml + skills.json + MEMORY.md + skills/ on disk · Manifest version: embedded in skills/v4-readiness/SKILL.md as of ${today}*
...
- This skill never modifies `claw.yml`, never opens a PR, never pulls upstream. It only reports.
...
| `V4_READINESS_NO_CONFIG` | `claw.yml` unreadable; fork not initialized | No |
...
- **Never auto-mutate the fork.** The skill is read-only. It does not edit `claw.yml`, does not open a PR, does not pull upstream. The upgrade decision belongs to the operator.
...
- **Empty `claw.yml` skills block (fresh fork)** — verdict is `READY`, every bucket is empty. Article writes, notification fires with the single-line body. The operator confirms the fork has nothing to migrate.
