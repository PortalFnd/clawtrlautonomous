...
Upstream `claw.yml` ships with effectively only `heartbeat: enabled: true`. Every fresh fork inherits that default. If we score "skills enabled" across **all active forks**, the leaderboard is a tautology (heartbeat always wins, everything else hovers near 0) and operator learns nothing. The lever is to score against **configured forks only** — forks whose `claw.yml` diverges from upstream defaults — and to convert the result into three actionable recommendations: which skills to promote, which fleet patterns to copy upstream, which skills to sunset.
...
Handle errors:
- 404 / 409 (empty repo): mark `status: "no_tree"`, skip claw.yml and skills/ extraction, continue.
...
Then fetch the fork's `claw.yml` only if the tree contains it:

```bash
gh api "repos/${FORK_FULL}/contents/claw.yml" --jq '.content' | base64 -d
```

If the path is in the tree but the contents call 404s, mark `status: "yml_unreadable"` and continue.

Extract from each readable `claw.yml`:
...
- **TEMPLATE**: claw.yml is readable but every diff signal is 0. Untouched template — exclude from leaderboard math.
- **UNREADABLE**: no_tree / no claw.yml / yml_unreadable / rate_limited. Tracked in the source-status footer.
...
*Scanned ${N_ACTIVE} active forks of ${TARGET_REPO} (pushed in last 30 days). ${N_CONFIGURED} are configured (claw.yml diverges from upstream defaults). Leaderboard scored against the configured ${N_CONFIGURED}.*
...
| Template (untouched claw.yml) | N_TEMPLATE | XX% |
...
- claw.yml readable: (N_CONFIGURED + N_TEMPLATE) / N_ACTIVE
- Rate-limited: N_RATE_LIMITED
- Fork-only skill files inspected: N_FORK_ONLY_FILES

---
*Source: GitHub API — forks of ${TARGET_REPO}. Methodology: a fork counts as "configured" if its `claw.yml` differs from upstream defaults on `enabled`, `model`, `var`, or `schedule` for any skill. Untouched templates are excluded from leaderboard math.*
```
